...
class CoverStorage(context: Context) {
...
//        )
    ...
    fun insertOrReplace(cover: Cover) {
    ...
    fun insertOrReplace(list: List<Cover>) {
        list.map { cover ->
            insertOrReplace(cover)
        }
    }

    fun getCoverById(coverId: Long): Cover? =
        coverList.find { it.id == coverId }

    fun getAll(): List<Cover> = coverList
